...
# aspect が記録されていない古いデータに使う既定の縦横比。
#
# collect.html は 1280x960 (4:3) を要求しているが、**実機は 1280x720 (16:9) を返す**。
# 2026-08-05に撮った壬のデータには aspect=1.7778 が記録されており、それが根拠。
# 2026-07-31以前のデータには aspect が入っていないが、同じPC・同じツールで
# 撮っているので同じ16:9とみなす。
# 要求した解像度ではなく実際に届く解像度で決まる点に注意。
# 画面の「カメラ」欄に実測値が出るので、違っていたらここを直して作り直すこと。
TRAIN_ASPECT = 16 / 9
