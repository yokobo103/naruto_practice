...
# aspect が記録されていないデータに使う最後の手段の既定値。
#
# いまは dataset/ の全データに aspect が入っているので、通常この値は使われない。
# サンプルごとに記録された値が常に優先される。
#
# 経緯: collect.html は 1280x960 (4:3) を要求しているが、**実機は 1280x720 (16:9)
# を返していた**。要求した解像度ではなく実際に届く解像度で決まる。
# これを4:3と誤認していた間、学習時と実機で1.33倍の歪みが入っていた。
# 2026-08-05に画面の「カメラ」欄で 1.778 を実測して確定し、
# 記録の無かった2026-07-31のデータにも後から補記した。
TRAIN_ASPECT = 16 / 9
